...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:sxx="http://www.SoundExchange/ns/xslt/util" exclude-result-prefixes="#all"
   ...
   <!-- Module mapping Ingrooves DDEX messages into Release ID Canonical (definitively comparable) format.
      
        NOT FOR STANDALONE USE! Called in by generate-canonical.xsl -->
 
   <!-- Ingrooves message -->
   <xsl:template match="*:NewReleaseMessage[$provider='Ingrooves']" mode="generate-canonical" priority="1">
   ...
   <!-- Ingrooves release -->
   <xsl:template match="*:NewReleaseMessage[$provider='Ingrooves']//Release" mode="release" priority="1">
      ...
      <xsl:call-template name="generate-canonical">
         ...
         <xsl:with-param tunnel="yes" name="releaseOriginalDate" select="$Release_Details/(OriginalReleaseDate,OriginalDigitalReleaseDate)[1]"/>
   ...
   <!-- Ingrooves recording -->
   <xsl:template match="SoundRecording[$provider='Ingrooves']" mode="recording">
      ...
      <xsl:call-template name="generate-recording">
         ...
         <xsl:with-param tunnel="yes" name="recordingLabel" select="$Recording_Details/LabelName"/>
