...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:sxx="http://www.SoundExchange/ns/xslt/util" exclude-result-prefixes="#all"
   ...
   <!-- Module mapping Orchard DDEX messages into Release ID Canonical (definitively comparable) format.
      
        NOT FOR STANDALONE USE! Called in by generate-canonical.xsl -->
   
   <!-- Orchard message -->
   <xsl:template match="*:NewReleaseMessage[$provider='Orchard']" mode="generate-canonical" priority="1">
   ...
   <!-- Orchard release -->
   <xsl:template match="*:NewReleaseMessage[$provider='Orchard']//Release" mode="release" priority="1">
      ...
      <xsl:call-template name="generate-canonical">
         ...
         <xsl:with-param tunnel="yes" name="releaseArtists" select="$Release_Details/
            (if (exists(DisplayArtistName)) then DisplayArtistName
             else DisplayArtist[ArtistRole='MainArtist'])"/>
         <xsl:with-param tunnel="yes" name="releaseLabel" select="$Release_Details/LabelName"/>
         <xsl:with-param tunnel="yes" name="releaseOriginalDate" select="$Release_Details/OriginalReleaseDate"/>
         <xsl:with-param tunnel="yes" name="releaseGenre" select="$Release_Details/Genre[1]/GenreText"/>
   ...
   <!-- Orchard recording -->
   <xsl:template match="SoundRecording[$provider='Orchard']" mode="recording">
      <xsl:param tunnel="yes" name="releaseLabel" required="yes"/>
      ...
      <xsl:call-template name="generate-recording">
         ...
         <xsl:with-param tunnel="yes" name="recordingArtists" select="$Recording_Details/DisplayArtist[ArtistRole='MainArtist']/PartyName"/>
         <xsl:with-param tunnel="yes" name="recordingLabel" select="($Recording_Details/LabelName,$releaseLabel)[1]"/>
         <xsl:with-param tunnel="yes" name="recordingDuration" select="Duration"/>
         <xsl:with-param tunnel="yes" name="recordingGenre" select="$Recording_Details/Genre[1]/GenreText"/>
      </xsl:call-template>
   </xsl:template>
