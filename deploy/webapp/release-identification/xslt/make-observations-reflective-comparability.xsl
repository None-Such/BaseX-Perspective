...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="observations">
      <xsl:variable name="tests" as="element()*">
         <sxx:reflective-comparability.canonical.is-comparable/>
      </xsl:variable>
      <xsl:copy>
   ...
   <xsl:template match="observation">
      <xsl:copy-of select="."/>
      <xsl:variable name="tests" as="element()*">
         <sxx:reflective-comparability.canonical.is-not-comparable/>
      </xsl:variable>
      <xsl:apply-templates select="$tests">
         <xsl:with-param name="context" tunnel="yes" select="."/>
      </xsl:apply-templates>
   ...
   <xsl:template match="sxx:reflective-comparability.canonical.is-comparable">
   ...
   <xsl:template match="sxx:reflective-comparability.canonical.is-not-comparable">
      <xsl:param name="context" required="yes" tunnel="yes"/>
